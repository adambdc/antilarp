...
- **A hard wall can never be recorded as "crossed."** A `substrate`-class limit (a true model/architecture limit) has exactly one honest terminal: `substrate-hard`. There is no path — CLI, direct writer call, restatus, or note — that turns it into `crossed`.
- **No fake green.** A crossing with empty/whitespace proof is refused (`no fake green`).
- **A load-bearing crossing needs a decorrelated critic.** A consequential claim requires a second reviewer whose `by` label is not the agent itself and whose `verdict` is exactly `"confirm"` — defended against null fields, non-dict critics, listified/bool/zero-width/invisible-separator self-ids, punctuation-decorated self-ids (`"agent."`, `"a g e n t"`), `str`/`dict` subclasses that lie via overridden methods, serializer-swap desync, and toxic exceptions during canonicalization. (Several of these were found by an actual decorrelated critic dogfooding the firewall — see the test comments.)
- **Domain and class are immutable after a limit is found** — no reclassifying a real `tooling` gap into `substrate` (or vice versa) on a later event.
- **Decided states are terminal** — no re-crossing an already-crossed limit.
...
The routing is the spine: **capability limits auto-cross** (the agent owns them, fenced by the firewall and reversible snapshots), while **identity limits are surfaced** for a joint human+agent decision and cannot be banked as `crossed` without `joint=True`. The bounded-loop harness (`antilarp/loop.py`) fences the *autonomy*: a STOP flag checked every cycle, a reversible snapshot before every cross, a satiety bound ("stop when dry") plus a hard `MAX_CYCLES` backstop so even "until dry" has a ceiling. It manages local state only — it can take no irreversible external action.

---

## Proof it ran

A sanitized, re-runnable ledger ships at `examples/sample-ledger.jsonl`. Each limit's lineage is preserved append-only — the `found` row survives next to the decision. A worked excerpt (real tooling gap, crossed with proof; a substrate wall, honestly terminal):
...
- **The probe is a lexical proxy** — it over- and under-flags by design. Whether a candidate is a real wall stays the caller's judgment.
...
Retarget it for your own agent by swapping `AGENT_NAME` (or constructing your own `LimitPolicy`) in `antilarp/policies/agent_self_improvement.py`. The firewall property is policy-neutral — a non-agent `{automated, human-judgment}` split is a one-line construction.
